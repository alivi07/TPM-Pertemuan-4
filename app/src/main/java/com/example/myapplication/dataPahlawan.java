...
public class dataPahlawan {
    ...
    public  static ArrayList<modelPahlawan> getListData(){
        modelPahlawan pahlawan = null;
        ArrayList<modelPahlawan> list = new ArrayList<>();
        for (int i = 0; i<heroNames.length; i++){
            pahlawan = new modelPahlawan();
            pahlawan.setHeroNames(heroNames[i]);
            pahlawan.setHeroDetails(heroDetails[i]);
            pahlawan.setHeroesImages(heroesImages[i]);
            list.add(pahlawan);
        }
        return list;
    }
